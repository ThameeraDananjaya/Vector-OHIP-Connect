...
import { Play, CheckCircle, AlertCircle, Loader2, Download, Upload, Eye, EyeOff } from 'lucide-react';
...
function extractFieldsForExcel(obj: any, path = '', displayPath = ''): FieldInfo[] {
  ...
  if (Array.isArray(obj)) {
    ...
    if (obj.length > 0) {
      ...
      const childFields = extractFieldsForExcel(firstItem, `${path}[0]`, displayPath);
    ...
    for (const key of Object.keys(obj)) {
      ...
      const newDisplayPath = displayPath ? `${displayPath} > ${formatFieldName(key)}` : formatFieldName(key);
      const value = obj[key];
      ...
      if (value === null || value === undefined) {
        ...
        // Recurse into array
        const childFields = extractFieldsForExcel(value, newPath, newDisplayPath);
        fields.push(...childFields);
...
function buildNestedObjectFromExcel(data: Record<string, any>, templateObj: any): any {
...
export function PayloadBuilder({ endpoint, collection, environment }: PayloadBuilderProps) {
  ...
  useEffect(() => {
    ...
    if (queryString) {
      ...
      urlSearchParams.forEach((value, key) => {
        ...
        const resolvedValue = value.replace(/\{\{(\w+)\}\}/g, (_, varName) => config?.[varName] ?? '');
        initialQueryParams[key] = resolvedValue;
      });
    }
    
    // Then add/override with explicit queryParams from endpoint definition
    (safeEndpoint?.queryParams ?? []).filter((p: any) => !p?.disabled).forEach((p: any) => {
      if (p?.key) {
    ...
    (safeEndpoint?.headers ?? []).filter((h: any) => !h?.disabled).forEach((h: any) => {
      if (h?.key) {
  ...
  const handleDownloadTemplate = () => {
    try {
      ...
      const wsData: (string | number | boolean)[][] = [
        ['Field Name', 'Your Value', 'Sample/Default Value', 'JSON Path (Reference)']
      ];
    ...
    reader.onload = (e) => {
      try {
        ...
        toast({
          ...
          description: 'Excel data converted to JSON payload successfully. Review and click Submit.',
        });
      } catch (error: any) {
        toast({
  ...
  const enabledHeaders = (safeEndpoint?.headers ?? []).filter((h: any) => !h?.disabled);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b bg-muted/30">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="font-semibold text-lg truncate">{safeEndpoint?.name ?? 'Endpoint'}</h2>
        </div>
        <div className="flex items-center gap-2">
          ...
          {safeEndpoint?.description && (
            ...
            >
              {showDescription ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
              {showDescription ? 'Hide description' : 'Show description'}
            </button>
          )}
        </div>
        {showDescription && safeEndpoint?.description && (
          <p className="text-sm text-muted-foreground mt-2 p-2 bg-muted/50 rounded">{safeEndpoint.description}</p>
        )}
      </div>
          ...
          {urlParams.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {urlParams.map(param => (
                  <div key={param}>
                    <label className="text-sm font-medium mb-1 block">{param}</label>
                    <Input
                ...
                {queryParamKeys.map((paramKey: string) => (
                  <div key={paramKey}>
                    ...
                    <Input
                      ...
                      onChange={(e) => setQueryParams(prev => ({ ...(prev ?? {}), [paramKey]: e.target.value }))}
            ...
            <div>
              ...
              <div className="space-y-3">
                {enabledHeaders.map((header: any) => (
                  <div key={header?.key ?? ''}>
                    <label className="text-sm font-medium mb-1 block">{header?.key ?? ''}</label>
                    <Input
                      value={headers?.[header?.key ?? ''] ?? ''}
                      onChange={(e) => setHeaders(prev => ({ ...(prev ?? {}), [header?.key ?? '']: e.target.value }))}
            ...
            <div>
              ...
              {isPostMethod && (
                <p className="text-xs text-muted-foreground mt-2">
                  Tip: Download the Excel template, fill in your data, then upload it to auto-populate the payload.
                </p>
              )}
            </div>
          )}
